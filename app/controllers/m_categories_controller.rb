class MCategoriesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @m_category.save
        format.html { redirect_to(@m_category, :notice => 'M category was successfully created.') }
  ...
  def update
    ...
    respond_to do |format|
      if @m_category.update_attributes(params[:m_category])
        format.html { redirect_to(@m_category, :notice => 'M category was successfully updated.') }
        format.xml  { head :ok }
      else
